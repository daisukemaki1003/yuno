...
/**
 * オプション（出力整形やカラーON/OFF）
 */
interface LoggerOptions {
  /**
   * JSONのインデント幅。2 以上で pretty-print。
   * 例: 2（開発時は 2 を推奨）/ 0（本番でコンパクトに）
   */
  indent?: number;

  /**
   * 文字色を付与するか（開発時は true 推奨）
   */
  color?: boolean;

  /**
   * サービス名（既定: "cloud-functions"）
   */
  service?: string;
}
...
export class Logger {
  ...
  constructor(requestId: string, opts: LoggerOptions = {}) {
   ...
   */
  static withRequest(ctx: Context | Request, opts?: LoggerOptions): Logger {
