...
describe('MeetingBaasAdapterV1', () => {
  ...
  describe('leaveBot', () => {
    test('should send leave request with correct parameters', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        'https://api.test.com/v1/bots/bot-123/leave',
        expect.objectContaining({
          method: 'POST',
          body: JSON.stringify({ meetingId: 'meeting-456' }),
        })
  ...
  describe('configuration flexibility', () => {
    test('should support different auth schemes', async () => {
      const configs = [
        { scheme: 'ApiKey' as const, expected: 'ApiKey test-api-key' },
        { scheme: 'Basic' as const, expected: 'Basic test-api-key' },
        { scheme: 'None' as const, expected: 'test-api-key' },
      ];
  ...
  describe('configuration', () => {
    test('should support different status mappings', async () => {
      const customConfig: MeetingBaasConfig = {
        ...
        maps: {
          status: {
            'PENDING': 'joining' as const,
            'CONNECTED': 'joined' as const,
            'TERMINATING': 'leaving' as const,
            'TERMINATED': 'left' as const,
